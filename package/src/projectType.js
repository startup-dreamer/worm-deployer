...
export async function determineProjectType() {
  const hasHardhatConfig =
    fs.existsSync(path.join(process.cwd(), "hardhat.config.js")) ||
    fs.existsSync(path.join(process.cwd(), "hardhat.config.ts"));
  const hasFoundryToml = fs.existsSync(
    path.join(process.cwd(), "foundry.toml")
  );
  ...
  if (hasHardhatConfig && hasFoundryToml) {
    ...
    throw new Error(
      chalk.red("Neither Hardhat nor Foundry configuration found.")
    );
  }
}

...
export async function getDeploymentDetails(deploymentType) {
    ...
    const { chain } = await inquirer.prompt([
      {
        ...
        choices: wormholeConfig.chains.map((chain) => ({
          name: chalk.yellow(chain),
          value: chain,
        })),
      },
    ]);
    ...
    const { chain } = await inquirer.prompt([
      {
        ...
        name: "chain",
        message: chalk.cyan(
          "Choose the source chain (fees for deployment will be taken from here):"
        ),
        choices: wormholeConfig.chains.map((chain) => ({
          name: chalk.yellow(chain),
          value: chain,
    ...
    const { destinations } = await inquirer.prompt([
      {
        ...
        choices: wormholeConfig.chains
          ...
          .map((chain) => ({ name: chalk.green(chain), value: chain })),
      },
    ]);
    ...
    const { deploySource } = await inquirer.prompt([
      {
        ...
        name: "deploySource",
        message: chalk.cyan(
          "Do you want to deploy on the source chain as well?"
        ),
        default: false,
      },
    ]);
  ...
  const { hasEnvFile } = await inquirer.prompt([
    {
      ...
      name: "hasEnvFile",
      message: chalk.cyan(
        "Is there a .env file present in the project directory?"
      ),
    },
  ]);
  let spinner;
  if (hasEnvFile) {
    ...
    if (!privateKey) {
      ...
      console.log(
        chalk.yellow(
          "No private key found in .env file. Please enter it manually."
        )
      );
